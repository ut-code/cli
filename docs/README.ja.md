...
WebSocket を介してクライアントとプログラマーがリアルタイムで質疑応答を行うための CLI ツールです。

プログラマーはキューに自分を登録し、クライアントはキューを閲覧して好みのプログラマーに接続します。
...
│   └── src/
│       ├── main.rs
│       ├── ask.rs   # クライアント: キュー閲覧、プログラマー選択、質問送信
│       └── serve.rs # プログラマー: キュー登録、質問への回答
...
利用可能なプログラマーの一覧を取得し、1 人を選択してセッションを開始します。プロンプトに質問を入力すると、回答がリアルタイムで届きます。終了するには `/quit` を入力するか `Ctrl+D` を押してください。
...
| メソッド | パス | 説明 |
|--------|------|-------------|
| `GET` | `/queue` | 待機中のプログラマー一覧 `{ roomId: label }` |
| `POST` | `/queue` | プログラマーを登録 `{ label }` → `{ roomId }` |
| `DELETE` | `/queue/:roomId` | プログラマーを登録解除 |
| `GET` (WS) | `/rooms/:id/programmer` | プログラマー用 WebSocket |
| `GET` (WS) | `/rooms/:id/client` | クライアント用 WebSocket |

ワーカーは 2 つの Durable Object で支えられています。

- **`QueueDO`** — シングルトン。待機中のプログラマーのキューを KV ストレージに永続化
- **`RoomSession`** — ルームごとに 1 つ。ハイバネータブル WebSocket を使用してプログラマーとクライアント間のメッセージを中継
